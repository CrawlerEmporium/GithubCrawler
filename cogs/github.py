...
class Github(commands.Cog):
    ...
    async def resolve(self, ctx, _id, *, msg=''):
        ...
        if (ctx.guild.id == GG.GUILD and ctx.message.author.id == GG.GIDDY) or \
                ...
                (ctx.guild.id == GG.FMP and ctx.message.author.id == GG.THEO) or \
    ...
    async def reidentify(self, ctx, report_id, identifier):
        ...
        if (ctx.guild.id == GG.GUILD and ctx.message.author.id == GG.GIDDY) or \
            ...
            elif ctx.guild.id == GG.FMP:
                msg = await ctx.bot.get_channel(GG.TRACKER_CHAN_FMP).send(embed=new_report.get_embed())
