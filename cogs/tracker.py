...
class Tracker(commands.Cog):
    ...
    async def server(self, ctx, threshold: int = 5):
        guild = ctx.guild
        server = guild.id
        ...
        if exist is None:
            gh = Server(name, server, admin, None, [], threshold)
    ...
    async def channel(self, ctx, type: str, identifier: str, tracker: int = 0, channel: int = 0):
        ...
        # CREATE CHANNELS
        if channel == 0:
            channel = await ctx.guild.create_text_channel("Listener")
        else:
            channel = self.bot.get_channel(channel)

        if tracker == 0:
            tracker = await ctx.guild.create_text_channel("Tracker")
        else:
            tracker = self.bot.get_channel(tracker)

        # ADD LISTENER TO THE SERVER
        await GG.MDB.ReportNums.insert_one({"key": identifier, "amount": 0})
        listener = Listen(channel.id, tracker.id, identifier, None)
    ...
    async def info(self, ctx):
        ...
        for listen in server.listen:
            channels += f"Listening to: {self.bot.get_channel(listen.channel).mention}\nPosting to: {self.bot.get_channel(listen.tracker).mention}\nUsing Identifier: ``{listen.identifier}``\n\n"
